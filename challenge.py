...
if __name__ == '__main__':
    '''
    Call the scoring function by looping through the models we defined.
    Models starting with Tf are tensorflow estimnators
    Please comment out the model you do not wish to be run 
    models ~ models that are going to be tested 
    best ~ variable used to store the most performant model 
    m ~ helper used for comparaison 
    res ~ Dictionary used to store the result, print the results, and find the best estimator 
'''
    models= [('LogisticRegression',LogisticRegression()),
             ...
             ('tfLinearClassifier', tfLinearClassifier()),
             ('tfDNNClassifier',tfDNNClassifier()),
            ('cus',tfCustomDNNEstimator())
             ]
    res= {}
    for e in models:
        res[e[0]]= [e ,score_solution(e)]
    ...
    #saving the best model on file
    #score_solution(res[best][0], save= 1)
        
     #loading a saved model
     
    #loaded_model = joblib.load('Best_Estimator.sav')
